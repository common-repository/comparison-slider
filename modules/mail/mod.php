<?php
class mailIcs extends moduleIcs {
	private $_smtpMailer = null;
	private $_sendMailMailer = null;
	
	public function init() {
		parent::init();
		//dispatcherIcs::addFilter('optionsDefine', array($this, 'addOptions'));
	}
	public function send( $to, $subject, $message, $fromName = '', $fromEmail = '', $replyToName = '', $replyToEmail = '', $additionalHeaders = array(), $additionalParameters = array() ) {
		$type = frameIcs::_()->getModule('options')->get('mail_send_engine');
		$res = false;
		switch($type) {
			case 'smtp':
	...
	private function _getSmtpMailer() {
		if (!$this->_smtpMailer) {
			...
			$smtpSecure = frameIcs::_()->getModule('options')->get('smtp_secure');
			if (!empty($smtpSecure)) {
				$this->_smtpMailer->SMTPSecure = $smtpSecure; // secure transfer enabled REQUIRED for GMail
			}
			$this->_smtpMailer->Host = trim(frameIcs::_()->getModule('options')->get('smtp_host'));
			$this->_smtpMailer->Port = trim(frameIcs::_()->getModule('options')->get('smtp_port')); 
			$this->_smtpMailer->Username = trim(frameIcs::_()->getModule('options')->get('smtp_login'));  
			$this->_smtpMailer->Password = trim(frameIcs::_()->getModule('options')->get('smtp_pass'));
		}
	...
	private function _getSendMailMailer() {
		if (!$this->_sendMailMailer) {
			...
			$sendMailPath = trim(frameIcs::_()->getModule('options')->get('sendmail_path'));
	...
	public function sendSendMailMail( $to, $subject, $message, $fromName = '', $fromEmail = '', $replyToName = '', $replyToEmail = '', $additionalHeaders = array(), $additionalParameters = array() ) {
		...
		if (frameIcs::_()->getModule('options')->get('disable_email_html_type')) {
		...
		switch ($type) {
			...
			case 'wp_mail': default:
				...
				if(empty($ts_mail_errors)) {
					$ts_mail_errors[] = __('Cannot send email - problem with send server', ICS_LANG_CODE);
				}
	...
	public function addOptions( $opts ) {
		$opts[ $this->getCode() ] = array(
			'label' => __('Mail', ICS_LANG_CODE),
			'opts' => array(
				'mail_function_work' => array('label' => __('Mail function tested and work', ICS_LANG_CODE), 'desc' => ''),
				'notify_email' => array('label' => __('Notify Email', ICS_LANG_CODE), 'desc' => __('Email address used for all email notifications from plugin', ICS_LANG_CODE), 'html' => 'text', 'def' => get_option('admin_email')),
			),
		);
		return $opts;
	}
