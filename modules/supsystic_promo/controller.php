<?php
class supsystic_promoControllerIcs extends controllerIcs {
    public function welcomePageSaveInfo() {
		$res = new responseIcs();
		installerIcs::setUsed();
		if ($this->getModel()->welcomePageSaveInfo(reqIcs::get('get'))) {
			$res->addMessage(__('Information was saved. Thank you!', ICS_LANG_CODE));
		} else {
			$res->pushError($this->getModel()->getErrors());
		}
		$originalPage = reqIcs::getVar('original_page');
		$http = isset($_SERVER['HTTPS']) && !empty($_SERVER['HTTPS']) ? 'https://' : 'http://';
		if (strpos($originalPage, $http. $_SERVER['HTTP_HOST']) !== 0) {
			$originalPage = '';
		}
		redirectIcs($originalPage);
	}
	public function sendContact() {
		$res = new responseIcs();
		$time = time();
		$prevSendTime = (int) get_option(ICS_CODE. '_last__time_contact_send');
			...
			if ($validate) {
				...
				foreach ($validate as $v) {
					...
					switch ($v) {
						case 'notEmpty':
							if (empty($data[ $fName ])) {
								$error = $fData['html'] == 'selectbox' ? __('Please select %s', ICS_LANG_CODE) : __('Please enter %s', ICS_LANG_CODE);
								$error = sprintf($error, $fData['label']);
							}
							break;
						case 'email':
							if (!is_email($data[ $fName ])) {
								$error = __('Please enter valid email address', ICS_LANG_CODE);
							}
							break;
					}
					if (!empty($error)) {
		...
		if (!$res->error()) {
			...
			$msg .= 'Plugin: ' . ICS_WP_PLUGIN_NAME . '<br />';
			foreach ($fields as $fName => $fData) {
			...
			if (frameIcs::_()->getModule('mail')->send('support@supsystic.zendesk.com', $data['subject'], $msg, $data['name'], $data['email'])) {
				update_option(ICS_CODE. '_last__time_contact_send', $time);
			} else {
				$res->pushError( frameIcs::_()->getModule('mail')->getMailErrors() );
			}
	...
	public function addNoticeAction() {
		$res = new responseIcs();
		$code = reqIcs::getVar('code', 'post');
		$choice = reqIcs::getVar('choice', 'post');
		if (!empty($code) && !empty($choice)) {
			$optModel = frameIcs::_()->getModule('options')->getModel();
			switch ($choice) {
				case 'hide':
	...
	public function addTourStep() {
		$res = new responseIcs();
		if ($this->getModel()->addTourStep(reqIcs::get('post'))) {
			$res->addMessage(__('Information was saved. Thank you!', ICS_LANG_CODE));
		} else {
			$res->pushError($this->getModel()->getErrors());
		}
	...
	public function closeTour() {
		$res = new responseIcs();
		if ($this->getModel()->closeTour(reqIcs::get('post'))) {
			$res->addMessage(__('Information was saved. Thank you!', ICS_LANG_CODE));
		} else {
			$res->pushError($this->getModel()->getErrors());
		}
	...
	public function addTourFinish() {
		$res = new responseIcs();
		if ($this->getModel()->addTourFinish(reqIcs::get('post'))) {
			$res->addMessage(__('Information was saved. Thank you!', ICS_LANG_CODE));
		} else {
			$res->pushError($this->getModel()->getErrors());
		}
	...
	public function saveDeactivateData() {
		$res = new responseIcs();
		if ($this->getModel()->saveDeactivateData(reqIcs::get('post'))) {
			$res->addMessage(__('Thank you for Feedback!', ICS_LANG_CODE));
		} else {
			$res->pushError($this->getModel()->getErrors());
		}
		$res->ajaxExec();
	...
	public function enbStatsOpt() {
		$res = new responseIcs();
		frameIcs::_()->getModule('options')->getModel()->save('send_stats', 1);
		$res->ajaxExec();
	}
	/**
	...
	public function getPermissions() {
		return array(
			ICS_USERLEVELS => array(
				ICS_ADMIN => array('welcomePageSaveInfo', 'sendContact', 'addNoticeAction', 
