<?php
class supsystic_promoModelIcs extends modelIcs {
	private $_apiUrl = '';
	...
	public function saveUsageStat( $code, $unique = false ) {
		...
		$query = 'INSERT INTO @__usage_stat SET code = "' . dbIcs::escape($code) . '", visits = 1
			ON DUPLICATE KEY UPDATE visits = visits + 1';
		return dbIcs::query($query);
	}
	private function _checkUniqueStat( $code ) {
		$uniqueStats = get_option(ICS_CODE . '_unique_stats');
		...
		$uniqueStats[] = $code;
		update_option(ICS_CODE . '_unique_stats', $uniqueStats);
		return false;
	}
	...
	public function getUserStatsCount() {
		...
		return (int) dbIcs::get($query, 'one');
	}
	...
	public function getTourHst() {
		$hst = get_user_meta(get_current_user_id(), ICS_CODE . '-tour-hst', true);
		if (empty($hst)) {
			$hst = array();
		}
		if (!isset($hst['passed'])) {
	...
	public function setTourHst( $hst ) {
		update_user_meta(get_current_user_id(), ICS_CODE . '-tour-hst', $hst);
	}
	public function clearTourHst() {
		delete_user_meta(get_current_user_id(), ICS_CODE . '-tour-hst');
	}
	...
	private function _getBigStatClient() {
		if (!$this->_bigCli) {
			...
			if (class_exists('Mixpanel')) {
				...
				/*$bigCliId = (int) get_option(ICS_CODE. '_big_cli_id');
				if(!$bigCliId) {
					$bigCliId = mt_rand(1, 9999999999);
					update_option(ICS_CODE. '_big_cli_id', $bigCliId);
				}
	...
	public function saveDeactivateData( $d ) {
		...
		$startUsage = (int) frameIcs::_()->getModule('options')->get('plug_welcome_show');
		if ($startUsage) {
