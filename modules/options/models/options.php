<?php
class optionsModelIcs extends modelIcs {
	...
	public function save( $optKey, $val, $ignoreDbUpdate = false ) {
		...
		if (!isset($this->_values[ $optKey ]) || $this->_values[ $optKey ]['value'] != $val) {
			...
			//frameIcs::_()->getModule('supsystic_promo')->getModel()->saveUsageStat('option.'. $optKey);
		}
	}
	...
	public function fillInValues( &$options ) {
		...
		foreach ($options as $cKey => $cData) {
			foreach ($cData['opts'] as $optKey => $optData) {
				...
				} else if (isset($optData['def'])) {	// If there were no saved data - set it as default
					$value = $optData['def'];
				}
    ...
    public function saveGroup( $d = array() ) {
		if (isset($d['opt_values']) && is_array($d['opt_values']) && !empty($d['opt_values'])) {
			dispatcherIcs::doAction('beforeSaveOpts', $d);
			...
			$this->pushError(__('Empty data to save option', ICS_LANG_CODE));
		}
        return false;
    }
	private function _updateOptsInDb() {
		update_option(ICS_CODE. '_opts_data', $this->_values);
	}
	private function _loadOptValues() {
		if (!$this->_valuesLoaded) {
			$this->_values = get_option(ICS_CODE. '_opts_data');
