jQuery(document).ready(function(){
	jQuery('#icsSettingsSaveBtn').click(function(){
		jQuery('#icsSettingsForm').submit();
		return false;
	});
	jQuery('#icsSettingsForm').submit(function(){
		jQuery(this).sendFormIcs({
			btn: jQuery('#icsSettingsSaveBtn')
		});
		return false;
	});
	...
	var $connectOpts = jQuery('#icsSettingsForm').find('[data-connect]');
	if($connectOpts && $connectOpts.size()) {
		var $connectedTo = {};
		$connectOpts.each(function(){
			var connectToData = jQuery(this).data('connect').split(':')
			,	$connectTo = jQuery('#icsSettingsForm').find('[name="opt_values['+ connectToData[ 0 ]+ ']"]')
