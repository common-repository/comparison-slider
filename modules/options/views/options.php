<?php
class optionsViewIcs extends viewIcs {
	private $_news = array();
	public function getNewFeatures() {
		$res = array();
		$readmePath = ICS_DIR . 'readme.txt';
		if (file_exists($readmePath)) {
			$readmeContent = @file_get_contents($readmePath);
			if (!empty($readmeContent)) {
				$matchedData = '';
				if (preg_match('/= ' . ICS_VERSION . ' =(.+)=.+=/isU', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				} else if (preg_match('/= ' . ICS_VERSION . ' =(.+)/is', $readmeContent, $matches)) {
					$matchedData = $matches[1];
				}
    ...
    public function getAdminPage() {
		...
		if (isset($tabs[ $activeTab ]) && isset($tabs[ $activeTab ]['callback'])) {
			//frameIcs::_()->getModule('supsystic_promo')->getModel()->saveUsageStat('tab.'. $activeTab);
		...
		frameIcs::_()->addJSVar('adminOptionsIcs', 'icsActiveTab', $activeTab);
		...
		$this->assign('breadcrumbs', frameIcs::_()->getModule('admin_nav')->getView()->getBreadcrumbs());
		$this->assign('mainLink', frameIcs::_()->getModule('supsystic_promo')->getMainLink());
	...
	public function getTabContent() {
		frameIcs::_()->addScript('admin.mainoptions', $this->getModule()->getModPath() . 'js/admin.mainoptions.js');
	...
	public function getSettingsTabContent() {
		frameIcs::_()->addScript('admin.settings', $this->getModule()->getModPath() . 'js/admin.settings.js');
		frameIcs::_()->getModule('templates')->loadJqueryUi();
		
		$options = frameIcs::_()->getModule('options')->getAll();
		$this->assign('options', $options);
		$this->assign('exportAllSubscribersUrl', uriIcs::mod('subscribe', 'getWpCsvList'));
