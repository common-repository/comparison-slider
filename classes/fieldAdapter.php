...
class fieldAdapterIcs {
	...
	static public function _( $fieldOrValue, $method, $type ) {
		if (method_exists('fieldAdapterIcs', $method)) {
			switch ($type) {
				case self::DB:
	...
	static public function userFieldDestHtml( $field ) {
		$field->htmlParams['optionsIcs'] = array();
	...
	static public function userFieldDestToDB( $value ) {
		return utilsIcs::jsonEncode($value);
	}
	static public function userFieldDestFromDB( $value ) {
		return utilsIcs::jsonDecode($value);
	}
	static public function taxDataHtml( $field ) {
		...
		$categories = frameIcs::_()->getModule('products')->getCategories();
		$brands = frameIcs::_()->getModule('products')->getBrands();
		$cOptions = array();
		$bOptions = array();
		if (!empty($categories)) {
		...
		return '<div>' . __('Apply To', ICS_LANG_CODE) . '
			<div id="tax_address">
				<b>' . __('Address', ICS_LANG_CODE) . '</b><br />
				' . __('Destination', ICS_LANG_CODE) . ':'. htmlIcs::checkboxlist('params[dest]', array('optionsIcs' => $listOfDest)) . '<br />
				' . __('Country', ICS_LANG_CODE) . ':'. htmlIcs::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])) . '<br />
			</div>
			<div id="tax_category">
				<b>' . __('Categories', ICS_LANG_CODE) . '</b><br />
				' . (empty($cOptions) ? __('You have no categories', ICS_LANG_CODE) : htmlIcs::checkboxlist('params[categories][]', array('optionsIcs' => $cOptions))) . '<br />
					<b>'. __('Brands', ICS_LANG_CODE) . '</b><br />
				'. (empty($bOptions) ? __('You have no brands', ICS_LANG_CODE) : htmlIcs::checkboxlist('params[brands][]', array('optionsIcs' => $bOptions))) . '<br />
			</div>
			<div>' . __('Tax Rate', ICS_LANG_CODE) . ': ' . htmlIcs::text('params[rate]', array('value' => $field->value['rate'])) .'</div>
			<div>' . __('Absolute', ICS_LANG_CODE) . ': ' . htmlIcs::checkbox('params[absolute]', array('checked' => $field->value['absolute'])) .'</div>
		</div>';
	}
	...
	static public function getCountries( $notSelected = false ) {
		...
		if (empty($options[ $notSelected ])) {
			...
			if ($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? __('Not selected', ICS_LANG_CODE) : __($notSelected);
			}
			foreach (self::$countries as $c) {
	...
	static public function getExtraFieldOptions( $field_id ) {
		...
		$options = frameIcs::_()->getModule('optionsIcs')->getHelper()->getOptions($field_id);
		if (!empty($options)) {
	...
	static public function getFieldAttributes( $params ) {
		...
		if (!empty($params->attr)) {
			foreach ($params->attr as $key => $value) {
				$output .= __($key) . ':<br />';
				$output .= htmlIcs::text('params[attr][' . $key . ']',array('value' => $value)) . '<br />';
			}
		} else {
				$output .= __('class', ICS_LANG_CODE) . ':<br />';
				$output .= htmlIcs::text('params[attr][class]', array('value' => '')) . '<br />';
				$output .= __('id', ICS_LANG_CODE) . ':<br />';
				$output .= htmlIcs::text('params[attr][id]', array('value' => '')) . '<br />';
		}
		return $output;
	}
	/**
	...
	static function productFieldCategories( $field ) {
		if (!empty($field->htmlParams['optionsIcs'])) {
			return;
		}
		...
		if(!empty($categories)) {
			if(!is_array($field->value['categories']))
					$field->value['categories'] = array();
			$field->htmlParams['optionsIcs'][0] = in_array(0,$field->value['categories'])?__('Deselect All'):__('Select All', ICS_LANG_CODE);
			foreach($categories as $c) {
				$field->htmlParams['optionsIcs'][$c->term_taxonomy_id] = $c->cat_name;
			}
		}*/
	}
	...
	static public function getCachedCountries( $clearCache = false ) {
		if (empty(self::$countries) || $clearCache) {
			self::$countries = frameIcs::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
		}
	...
	static public function getCachedStates( $clearCache = false ) {
		if (empty(self::$states) || $clearCache) {
			self::$states = frameIcs::_()->getTable('states')
				->leftJoin( frameIcs::_()->getTable('countries'), 'country_id' )
