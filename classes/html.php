...
class htmlIcs {
	...
	static public function img( $src, $usePlugPath = 1, $params = array('width' => '', 'height' => '', 'attrs' => '') ) {
		if($usePlugPath) $src = ICS_IMG_PATH. $src;
		return '<img src="' . $src . '" '
	...
	static public function ajaxfile( $name, $params = array('url' => '', 'value' => '', 'fid' => '', 'buttonName' => '') ) {
		frameIcs::_()->addScript('ajaxupload', ICS_JS_PATH . 'ajaxupload.js');
		$out = '';
		if (strpos($params['url'], 'pl=' . ICS_CODE) === false) {
			$params['url'] = uriIcs::_(array('baseUrl' => $params['url'], 'pl' => ICS_CODE));
		}
	...
	static public function statesInput( $name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true, 'id' => '', 'selectHtml' => '') ) {
		...
		$states = fieldAdapterIcs::getStates($params['notSelected']);
		...
		if (empty($params['doNotAddJs'])) {
			$res .= '<script type="text/javascript">
				// <!--
				if(!toeStates.length)
					toeStates = ' . utilsIcs::jsonEncode($states) . ';
	...
	static public function countryList( $name, $params = array('value' => '', 'attrs' => '', 'notSelected' => true) ) {
		$params['notSelected'] = isset($params['notSelected']) ? $params['notSelected'] : true;
		$params['options'] = fieldAdapterIcs::getCountries($params['notSelected']);
	...
	static public function textFieldsDynamicTable( $name, $params = array('value' => '', 'attrs' => '', 'options' => array()) ) {
		...
		if (!empty($params['options'])) {
			...
			foreach ($params['options'] as $key => $p) {
				$pattern[$key] = htmlIcs::text($name . '[][' . $key . ']');
			}
			...
			if (empty($params['value'])) {
				...
				$params['value'] = utilsIcs::jsonDecode($params['value']);
			...
			do {
				...
				foreach ($params['options'] as $key => $p) {
					...
					$res .= __($p['label']). htmlIcs::text($name . '[' . $i . '][' . $key . ']', $paramsForText);
				}
				$res .= $remove . '</div>';
				$i++;
	...
	static protected function _loadProductsOptions() {
		if (empty(self::$productsOptions)) {
			$products = frameIcs::_()->getModule('products')->getModel()->get(array('getFields' => 'post.ID, post.post_title'));
			if (!empty($products)) {
	...
	static public function slider( $name, $params = array('value' => 0, 'min' => 0, 'max' => 0, 'step' => 1, 'slide' => '') ) {
		...
		if (!empty($params)) {
			$paramsArr = array();
			foreach ($params as $k => $v) {
				if (in_array($k, array('attrs')) || strpos($k, '-')) {
					continue;
				}
	...
	static public function fontsList( $name, $params = array('value' => '') ) {
		...
		if (empty($options)) {	// Fill them only one time per loading
			foreach (fieldAdapterIcs::getFontsList() as $font)
				$options[ $font ] = $font;
		}
		$params['options'] = array();
		if (isset($params['default'])) {
			$params['options'] = array(ICS_DEFAULT => $params['default']);
		}
	...
	static public function slideInput( $name, $params = array('attrs' => '', 'checked' => false, 'id' => '') ) {
		...
		return '<a class="toeSlideShellIcs" href="#"' . $params['attrs'] . '>
			<span class="toeSlideButtIcs"></span>
			<span class="toeSlideOnIcs">' . __('ON') . '</span>
			<span class="toeSlideOffIcs">' . __('OFF') . '</span>
	...
	static public function galleryBtn( $name, $params = array() ) {
		...
		$buttonParams = $params;
		$buttonParams['value'] = isset($params['btnVal']) ? $params['btnVal'] : sprintf(__('Select %s', ICS_LANG_CODE), strFirstUp($galleryType));
