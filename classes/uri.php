...
class uriIcs {
	...
	static public function fileToPageParam( $file ) {
		...
		return substr($file, strpos($file, ICS_PLUG_NAME));
	}
	static public function _( $params ) {
		...
		if (is_string($params) 
			...
			|| strpos($params, ICS_PLUG_NAME) !== false)	// If relative links in WP is used (by other plugin for example)
		) {
			if (self::isHttps()) {
				$params = self::makeHttps($params);
			}
			return $params;
		} else if (is_array($params) && isset($params['page_id'])) {
			if (is_null($wp_rewrite)) {
				$wp_rewrite = new WP_Rewrite();
			}
			$link = get_page_link($params['page_id']);
			unset($params['page_id']);
		} else if (is_array($params) && isset($params['baseUrl'])) {
			$link = $params['baseUrl'];
