...
class frameIcs {
	...
	public function __construct() {
		$this->_res = toeCreateObjIcs('response', array());
		
	}
	static public function getInstance() {
		static $instance;
		if (!$instance) {
			$instance = new frameIcs();
		}
		return $instance;
	}
		...
		if ($activeModules) {
			foreach ($activeModules as $m) {
				...
				if (!empty($m['ex_plug_dir'])) {
					$moduleLocationDir = utilsIcs::getExtModDir( $m['ex_plug_dir'] );
				}
				if (is_dir($moduleLocationDir . $code)) {
					$this->_allModules[$m['code']] = 1;
					if ((bool)$m['active']) {
						importClassIcs($code . strFirstUp(ICS_CODE), $moduleLocationDir . $code . DS . 'mod.php');
						$moduleClass = toeGetClassNameIcs($code);
	...
	public function init() {
		...
		dispatcherIcs::doAction('afterModulesInit');
		
		modInstallerIcs::checkActivationMessages();
		...
		register_activation_hook(  ICS_DIR . DS . ICS_MAIN_FILE, array('utilsIcs', 'activatePlugin')  ); //See classes/install.php file
		register_uninstall_hook(ICS_DIR . DS . ICS_MAIN_FILE, array('utilsIcs', 'deletePlugin'));
		register_deactivation_hook(ICS_DIR . DS . ICS_MAIN_FILE, array( 'utilsIcs', 'deactivatePlugin' ) );

		add_action('init', array($this, 'connectLang'));
		//$operationTime = microtime(true) - $startTime;
	}
	public function connectLang() {
		load_plugin_textdomain(ICS_LANG_CODE, false, ICS_PLUG_NAME . '/lang');
	}
	/**
	...
	public function checkPermissions( $code, $action ) {
		if ($this->havePermissions($code, $action)) {
			...
			exit(_e('You have no permissions to view this page', ICS_LANG_CODE));
		}
	}
	/**
	...
	public function havePermissions( $code, $action ) {
		...
		if ($mod) {
			...
			if (!empty($permissions)) {  // Special permissions
				if (isset($permissions[ICS_METHODS]) 
					&& !empty($permissions[ICS_METHODS])
				) {
					foreach ($permissions[ICS_METHODS] as $method => $permissions) {   // Make case-insensitive
						$permissions[ICS_METHODS][strtolower($method)] = $permissions;
					}
					if (array_key_exists($action, $permissions[ICS_METHODS])) {        // Permission for this method exists
						$currentUserPosition = frameIcs::_()->getModule('user')->getCurrentUserPosition();
						if ((is_array($permissions[ ICS_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ ICS_METHODS ][ $action ]))
							|| (!is_array($permissions[ ICS_METHODS ][ $action ]) && $permissions[ICS_METHODS][$action] != $currentUserPosition)
						) {
							$res = false;
						}
					}
				}
				if (isset($permissions[ICS_USERLEVELS])
					&& !empty($permissions[ICS_USERLEVELS])
				) {
					$currentUserPosition = frameIcs::_()->getModule('user')->getCurrentUserPosition();
					// For multi-sites network admin role is undefined, let's do this here
					if (is_multisite() && is_admin() && is_super_admin()) {
						$currentUserPosition = ICS_ADMIN;
					}
					foreach ($permissions[ICS_USERLEVELS] as $userlevel => $methods) {
						if (is_array($methods)) {
			...
			if ($res) {	// Additional check for nonces
				...
				if (!empty($noncedMethods)) {
					...
					if (in_array($action, $noncedMethods)) {
						...
						check_ajax_referer('ics-nonce', 'icsNonce');
	...
	protected function _extractTables( $tablesDir = ICS_TABLES_DIR ) {
	...
	protected function _extractTable( $tableName, $tablesDir = ICS_TABLES_DIR ) {
		importClassIcs('noClassNameHere', $tablesDir . $tableName . '.php');
		$this->_tables[$tableName] = tableIcs::_($tableName);
	}
	/**
	 ...
	 * @return object table
	 * @example frameIcs::_()->getTable('products')->getAll()
	 */
			...
			foreach ($this->_modules as $code => $mod) {
				if (isset($filter['type'])) {
					if (is_numeric($filter['type']) && $filter['type'] == $mod->getTypeID()) {
						$res[$code] = $mod;
					} else if ($filter['type'] == $mod->getType()) {
						$res[$code] = $mod;
					}
				}
			}
	...
	public function addScript( $handle, $src = '', $deps = array(), $ver = false, $in_footer = false, $vars = array() ) {
		$src = empty($src) ? $src : uriIcs::_($src);
		if (!$ver) {
			$ver = ICS_VERSION;
		}
	...
	/**
	 * Add all scripts from _scripts array to woricsess
	 */
	...
	public function isTplEditor() {
		$tplEditor = reqIcs::getVar('tplEditor');
	...
	public function isAdminPlugOptsPage() {
		$page = reqIcs::getVar('page');
		if (is_admin() && strpos($page, frameIcs::_()->getModule('adminmenu')->getMainSlug()) !== false) {
			return true;
		}
		return false;
	}
	...
	public function savePluginActivationErrors() {
		update_option(ICS_CODE . '_plugin_activation_errors',  ob_get_contents());
	}
	public function getActivationErrors() {
		return get_option(ICS_CODE . '_plugin_activation_errors');
	}
}
