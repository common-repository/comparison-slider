...
class validatorIcs {
	...
	static public function validate( $field, $label = '', $validate = array() ) {
		self::$errors = array();
		if (is_object($field) && get_class($field) != 'fieldIcs') {
			$value = $field;
			$field = new fieldIcsIcs('noMatter');
			$field->label = $label;
		...
		if (!empty($field->validate)) {
			foreach ($field->validate as $v) {
				if (method_exists('validatorIcs', $v)) {
					self::$v($field);
				}
			}
		}
		if (method_exists('validatorIcs', $field->type)) {
			$validate = $field->type;
			self::$validate($field);
		}
		if($field->maxlen) {
	...
	static public function email( $field ) {
		if (!is_email($field->value)) {
			...
			return false;
		} else if (email_exists($field->value)) {
	...
	static public function getProductValidationMethods() {
		$res = array();
		$all = get_class_methods('validatorIcs');
		foreach ($all as $m) {
