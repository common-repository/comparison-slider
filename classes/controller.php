...
abstract class controllerIcs {
	...
	protected function _createModel( $name = '' ) {
		...
		$parentModule = frameIcs::_()->getModule( $this->getCode() );
		$className = '';
		if (importIcs($parentModule->getModDir() . 'models' . DS . $name . '.php')) {
			$className = toeGetClassNameIcs($name . 'Model');
		}
	...
	protected function _createView( $name = '' ) {
		...
		if (importIcs($parentModule->getModDir() . 'views' . DS . $name . '.php')) {
			$className = toeGetClassNameIcs($name . 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(ICS_ADMIN),
				'remove' => array(ICS_ADMIN),
				'restore' => ICS_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - ICS_METHODS or ICS_USERLEVELS
	 */
	...
	public function getModule() {
		return frameIcs::_()->getModule( $this->getCode() );
	}
	...
	public function getListForTbl() {
		...
		$page = (int) reqIcs::getVar('page');
		$rowsLimit = (int) reqIcs::getVar('rows');
		$orderBy = reqIcs::getVar('sidx');
		$sortOrder = reqIcs::getVar('sord');

		// Our custom search
		$search = reqIcs::getVar('search');
		...
		// jqGrid search
		$isSearch = reqIcs::getVar('_search');
		if ($isSearch) {
			$searchField = trim(reqIcs::getVar('searchField'));
			$searchString = trim(reqIcs::getVar('searchString'));
		...
		$res = dispatcherIcs::applyFilters($this->getCode() . '_getListForTblResults', $res);
		$res->ajaxExec();

	}
	public function removeGroup() {
		$res = new responseIcs();
		if ($this->getModel()->removeGroup(reqIcs::getVar('listIds', 'post'))) {
