...
class langIcs {
	...
	static public function extract( $d = array('dir' => '', 'langIcs' => '') ) {
		$data = array();
		if (isset($d['dir']) && !empty($d['dir'])) {
			$langDirPath = $d['dir'];
		} else if (isset($d['langIcs']) && !empty($d['langIcs'])) {
			$langDirPath = ICS_LANG_DIR . $d['langIcs'] . DS;
		} else {
			$langDirPath = ICS_LANG_DIR . ICS_WPLANG . DS;
		}
