<?php
abstract class moduleIcs extends baseObjectIcs {
	...
	public function __construct( $d/*, $params = array()*/ ) {
		...
		if (isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( utilsIcs::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( utilsIcs::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if (empty($this->_externalDir)) {
			return ICS_MODULES_DIR . $this->getCode() . DS;
		} else {
	...
	public function getModPath() {
		if (empty($this->_externalPath)) {
			return ICS_MODULES_PATH . $this->getCode() . '/';
		} else {
	...
	protected function _createController() {
		...
		if (file_exists($this->getModDir() . 'controller.php')) {
			$className = '';
			if (importIcs($this->getModDir() . 'controller.php')) {
				$className = toeGetClassNameIcs($this->getCode() . 'Controller');
			}
			if (!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir() . 'helper.php')) {
			...
			importClassIcs($helper, $this->getModDir() . 'helper.php');
