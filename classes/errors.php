...
class errorsIcs {
	...
	static public function setSession( $error ) {
		...
		reqIcs::setVar('sesErrors', $sesErrors, 'session');
	}
	static public function init() {
		$icsErrors = reqIcs::getVar('icsErrors');
		if (!empty($icsErrors)) {
			if (!is_array($icsErrors)) {
				$icsErrors = array( $icsErrors );
			}
			$icsErrors = array_map('htmlspecialchars', array_map('stripslashes', array_map('trim', $icsErrors)));
			if (!empty($icsErrors)) {
				self::$current = $icsErrors;
				if (is_admin()) {
					add_action('admin_notices', array('errorsIcs', 'showAdminErrors'));
				} else {
					add_filter('the_content', array('errorsIcs', 'appendErrorsContent'), 99999);
				}
			}
		}
	}
	...
	static public function getSession() {
		return reqIcs::getVar('sesErrors', 'session');
	}
	static public function clearSession() {
		reqIcs::clearVar('sesErrors', 'session');
	}
