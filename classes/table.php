...
abstract class tableIcs {
	...
	static public function getInstance( $table = '' ) {
		...
		if (!isset($instances[$table])) {
			$class = 'table' . strFirstUp($table) . strFirstUp(ICS_CODE);
			...
			if ($id) {
				...
				foreach ($data as $field) {
					...
					foreach ($field as $k => $v) {
						if (isset($this->_fields[$k])) {
							$row[$k] = toeCreateObjIcs('fieldIcs', array(
									$this->_fields[$k]->name,
									$this->_fields[$k]->html,
									$this->_fields[$k]->type,
	...
	public function getTable( $transform = false ) {
		if ($transform) {
			return dbIcs::prepareQuery($this->_table);
		} else {
			return $this->_table;
		}
	}
	...
	public function store( $data, $method = 'INSERT', $where = '' ) {
		...
		if (empty($fields)) {
			$this->_addError(__('Nothing to update', ICS_LANG_CODE));
			return false;
		}
		...
		if (dbIcs::query($query)) {
			...
			$this->_addError(ICS_TEST_MODE ? dbIcs::getError() : __('Database error. Please contact your developer.', ICS_LANG_CODE));
		}
		return false;
	}
	...
	public function _getQueryString( $data, $delim = ',', $validate = false ) {
		...
		if (is_array($data) && !empty($data)) {
			foreach ($data as $k => $v) {
				...
				} else if ($k == 'additionalCondition') {    //just add some string to query
					$res .= $v . ' ' . $delim . ' ';
				}
			}
	...
	/**
	 * Add new fieldIcsIcs for children table (@see class field)
	...
	protected function _addField( $name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '' ) {
		$this->_fields[$name] = toeCreateObjIcs('fieldIcs', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
		return $this;
	}
	/**
	...
	public function exists($value, $field = '') {
		...
		return dbIcs::get('SELECT ' . $this->_id . ' FROM ' . $this->_table . ' WHERE ' . $field . ' = "' . $value . '"', 'one');
	}
			...
			switch ($f->type) {
				case 'date':
					if ($d[$key] == '0000-00-00' || empty($d[$key])) {
						...
						$d[$key] = date(ICS_DATE_FORMAT, dbIcs::dateToTime($d[$key]));
					}
					break;
				case 'int':
				case 'tinyint':
	...
	public function getLastInsertID() {
		return dbIcs::get('SELECT MAX(' . $this->_id . ') FROM ' . $this->_table, 'one');
	}
