<?php
abstract class modelIcs extends baseObjectIcs {
	...
	public function getModule() {
		return frameIcs::_()->getModule( $this->_code );
	}
	...
	public function addWhere( $where ) {
		if (empty($this->_where) && !is_string($where)) {
			$this->setWhere( $where );
		} else if (is_array($this->_where) && is_array($where)) {
	...
	protected function _buildQuery( $table = null ) {
		if (!$table) {
			$table = frameIcs::_()->getTable( $this->_tbl );
		}
	...
	public function removeGroup( $ids ) {
		...
		if (!empty($ids)) {
			if (frameIcs::_()->getTable( $this->_tbl )->delete(array('additionalCondition' => 'id IN (' . implode(',', $ids) . ')'))) {
				return true;
			} else 
				$this->pushError (__('Database error detected', ICS_LANG_CODE));
		} else {
			$this->pushError(__('Invalid ID', ICS_LANG_CODE));
		}
		return false;
	}
	...
	public function delete( $params = array() ) {
		if (frameIcs::_()->getTable( $this->_tbl )->delete( $params )) {
			return true;
		} else {
			$this->pushError (__('Database error detected', ICS_LANG_CODE));
		}
	...
	public function insert( $data ) {
		...
		$id = frameIcs::_()->getTable( $this->_tbl)->insert( $data );
		if ($id) {
			return $id;
		}
		$this->pushError(frameIcs::_()->getTable( $this->_tbl )->getErrors());
		return false;
	}
	public function updateById( $data, $id = 0 ) {
		...
		if ($id) {
			...
			$this->pushError(__('Empty or invalid ID', ICS_LANG_CODE));
		}
		return false;
	}
	public function update( $data, $where ) {
		$data = $this->_dataSave($data, true);
		if (frameIcs::_()->getTable( $this->_tbl )->update( $data, $where )) {
			return true;
		}
		$this->pushError(frameIcs::_()->getTable( $this->_tbl )->getErrors());
		return false;
