...
class responseIcs {
	...
	public function ajaxExec( $forceAjax = false ) {
		$isAjax = $this->isAjax();
		$redirect = reqIcs::getVar('redirect');
		...
		/*if($redirect)
			redirectIcs($redirect);*/
		return $this;
	}
	public function mainRedirect( $redirectUrl = '' ) {
		$redirectUrl = empty($redirectUrl) ? ICS_SITE_URL : $redirectUrl;
		$redirectData = array();
		if (!empty($this->errors)) {
			$redirectData['icsErrors'] = $this->errors;
		}
		if (!empty($this->messages)) {
			$redirectData['icsMsgs'] = $this->messages;
		}
		return redirectIcs($redirectUrl. (strpos($redirectUrl, '?') ? '&' : '?'). http_build_query($redirectData));
	}
	...
	/**
	 * Alias for responseIcs::addError, @see addError method
	 */
