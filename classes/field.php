...
class fieldIcs {
	 ...
	 */
	public $adapt = array('htmlIcs' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue( $value, $fromDB = false ) {
		if (isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB) {
			$value = fieldAdapterIcs::_($value, $this->adapt['dbFrom'], fieldAdapterIcs::DB);
		}
		$this->value = $value;
	}
	...
	public function drawHtml( $tag, $id ) {
		if (method_exists('htmlIcs', $this->html)) {
			$method = $this->html;
			...
			if ($this->adapt['html']) {
				fieldAdapterIcs::_($this, $this->adapt['html'], fieldAdapterIcs::HTML);
			}
			...
			if ($this->name == 'default_value') {
				$optionsFromDb = frameIcs::_()->getModule('optionsIcs')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => __('Select', ICS_LANG_CODE));
					foreach ($optionsFromDb as $k => $v) {
						$options[$k] = $v;
					}
					$method = 'selectbox';
					$this->htmlParams['optionsIcs'] = $options;
				}
			}
			$htmlContent = htmlIcs::$method($this->name, $this->htmlParams);
			if (!empty($this->errorEl)) {
				if (is_bool($this->errorEl)) {
					$errorEl = '<div class="toeErrorForField toe_' . htmlIcs::nameToClassId($this->name) . '"></div>';
	...
	public function displayValue() {
		...
		switch ($this->html) {
			case 'countryList':
				$value = fieldAdapterIcs::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterIcs::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsIcs');
				$value = array();
	...
	public function viewField( $name, $fieldValue = '' ) {
		$method = $this->html;
		$options = frameIcs::_()->getModule('optionsIcs')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsIcs'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if (method_exists('htmlIcs', $method)) {
			$output .= htmlIcs::$method($name, $params);
			$output .= htmlIcs::hidden('extra_field[' . $this->name . ']', array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams( $tag, $id ) {
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = __('Add Checkbox', ICS_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = __('Add Item', ICS_LANG_CODE);
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = __('Add Radio Button', ICS_LANG_CODE);
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"' . $options_tag . '>';
					...
					$output .= fieldAdapterIcs::_($id, 'getExtraFieldOptions',fieldAdapterIcs::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"' . $image_tag . '>' .__('Dimensions', ICS_LANG_CODE) . ':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= __('width', ICS_LANG_CODE) . ':<br />';
					$output .= htmlIcs::text('params[width]', array('value'=>$width)) . '<br />';
					$output .= __('height', ICS_LANG_CODE) .':<br />';
					$output .= htmlIcs::text('params[height]', array('value'=>$height)) . '<br />';
				$output .= '</div>';
			}
			if ($this->adapt['htmlParams']) {
				$output .= fieldAdapterIcs::_($this, $this->adapt['htmlParams'], fieldAdapterIcs::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">' . __('Click to set field "id" and "class"', ICS_LANG_CODE) . '</a>';
				$output .= '<div class="attributes" style="display:none;">' . __('Attributes', ICS_LANG_CODE) . ':<br />';
				$output .= fieldAdapterIcs::_($params, 'getFieldAttributes',  fieldAdapterIcs::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam( $param, $element ) {
		return utilsIcs::xmlAttrToStr($param, $element);
	...
	public function prepareConfigOptions( $xml ) {
		...
		foreach ($config->params->param as $param) {
			...
			$helper = $this->checkVarFromParam($param, 'helperIcs');
			$module = $this->checkVarFromParam($param, 'moduleIcs');
	...
	public function drawConfig( $module, $additionalOptions = array() ) {
		if (!frameIcs::_()->getModule($module)) {
			return false; 
		}
		// check for xml file with params structure  
	   if (frameIcs::_()->getModule($module)->isExternal()) {
			$config_xml = frameIcs::_()->getModule($module)->getModDir() . 'mod.xml';
		} else {
			$config_xml = ICS_MODULES_DIR . $module . DS . 'mod.xml';
		}

		if (!file_exists($config_xml)) {
			// if there is no configuration file for this $module
			return __('There are no configuration options for this module', ICS_LANG_CODE);
		}
		$output = '';
		...
		} else if (is_object($this->value) || is_array($this->value)) {
		...
		if (!empty($configOptions)){
			...
			foreach ($params as $param) {
				...
				foreach ($configOptions as $key=>$value) {
					...
					// if helper is needed to render the object
					} else if ($configOptions[$key]['helper'] != '') {
						...
						$helper = frameIcs::_()->getModule($hmodule)->getHelper();
						if ($helper) {
					...
					$htmlParams = array('value'=>$fieldValue,'optionsIcs'=>$options);
					if ($method == 'checkbox') {
						$htmlParams['value'] = 1;
					...
					$output .= '</label><br />';
					$output .= htmlIcs::$method($name, $htmlParams) . '<br />';
					$output .= '</div>';
				}
				$i++;
				$output .= '</div>';
