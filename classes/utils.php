...
class utilsIcs {
	...
	static public function copyDirectories( $source, $destination ) {
		if (is_dir($source)) {
			...
			while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
				...
				if (is_dir($PathDir)) {
					utilsIcs::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
					continue;
				}
	...
	static public function xmlNodeAttrsToArr( $node ) {
		...
		foreach ($node->attributes() as $a => $b) {
			$arr[$a] = utilsIcs::xmlAttrToStr($node, $a);
		}
		return $arr;
	}
	...
	static public function isThisCommercialEdition() {
		...
		foreach($commercialModules as $m) {
			if(!frameIcs::_()->getModule($m)) 
				return false;
			if(!is_dir(frameIcs::_()->getModule($m)->getModDir())) 
				return false;
		}
		return true;*/
		foreach (frameIcs::_()->getModules() as $m) {
	...
	static public function isPluginsPage() {
		return (basename(reqIcs::getVar('SCRIPT_NAME', 'server')) === 'plugins.php');
	}
	...
	/**
	 * Parse woricsess post/page/custom post type content for images and return it's IDs if there are images
	...
	static public function getReferalUrl() {
		// Simple for now
		return reqIcs::getVar('HTTP_REFERER', 'server');
	}
