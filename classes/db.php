...
class dbIcs {
	 ...
	 * #__  Woricsess prefix
	 * ^__  Store plugin tables prefix (@see ICS_DB_PREF if config.php)
	...
	static public function prepareQuery( $query ) {
		...
		return str_replace(
				array('#__', '^__', '@__'), 
				array($wpdb->prefix, ICS_DB_PREF, $wpdb->prefix. ICS_DB_PREF),
				$query);
	}
	...
	static public function timeToDate( $timestamp = 0 ) {
		if ($timestamp) {
			if (!is_numeric($timestamp)) {
				$timestamp = dateToTimestampIcs($timestamp);
			}
	...
	static public function dateToTime( $date ) {
		...
		if (strpos($date, ICS_DATE_DL)) {
			return dateToTimestampIcs($date);
		}
		$arr = explode('-', $date);
		return dateToTimestampIcs($arr[2] . ICS_DATE_DL . $arr[1] . ICS_DATE_DL . $arr[0]);
	}
	static public function exist( $table, $column = '', $value = '' ) {
		...
		} else if (empty($value)) {                   //Check if column exist
