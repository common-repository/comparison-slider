	...
	var methods = {
		init : function( options ) {
			return this.each(function(){
				...
				if (!$this.hasClass('icsWpTabs')) {
					$this.addClass('icsWpTabs');
				...
				if($this.find(selector).size()) {
					...
					if(!this._firstInit) {
						if(this._options.uniqId)
							$this.find('.icsTabContent').attr('data-tabs-for', this._options.uniqId);
						this._firstInit = 1;
					}
					var allTabsContent = this._options.uniqId 
						? $this.find('.icsTabContent[data-tabs-for="'+ this._options.uniqId + '"]')
						: $this.find('.icsTabContent');
					allTabsContent.hide();
