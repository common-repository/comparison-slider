...
jQuery.fn.serializeAnythingIcs = function(addData, returnArray) {
...
function ics_str_replace(haystack, needle, replacement) { 
	var temp = haystack.split(needle); 
	return temp.join(replacement); 
}
/**
...
function toeListableIcs(params) {
	...
	this.draw = function(list, count) {
		this.table.find('tr').not('.icsExample, .icsTblHeader').remove();
		var exampleRow = this.table.find('.icsExample');
		for(var i in list) {
			...
			newRow.removeClass('icsExample').show();
...
function setCookieIcs(c_name, value, exdays) {
	var exdate = new Date();
 ...
 * @returns {jQuery}
 */
jQuery.fn.icsZoom = function(level, origin) {
	origin = origin ? origin : 'center center';
	jQuery(this).data('zoom', level);
	return jQuery(this).css({
	/*	'zoom': level	// Didn't worked correctly for mobiles
...
/**
 * Retrive woricsess attach ID from image, using img classes
...
function randIcs( min, max, returnFloat ) {
