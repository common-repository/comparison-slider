var icsAdminFormChanged = [];
window.onbeforeunload = function(){
	// If there are at lease one unsaved form - show message for confirnation for page leave
	if(icsAdminFormChanged.length)
		return 'Some changes were not-saved. Are you sure you want to leave?';
};
jQuery(document).ready(function(){
	icsInitMainPromoPopup();
	if(typeof(icsActiveTab) != 'undefined' && icsActiveTab != 'main_page' && jQuery('#toplevel_page_ics-comparison-slider').hasClass('wp-has-current-submenu')) {
		var subMenus = jQuery('#toplevel_page_ics-comparison-slider').find('.wp-submenu li');
		subMenus.removeClass('current').each(function(){
			if(jQuery(this).find('a[href$="&tab='+ icsActiveTab+ '"]').size()) {
	...
	setTimeout(function() {
		...
		if(formsPreventLeave && formsPreventLeave.length) {
			...
			jQuery('#'+ formsPreventLeave.join(', #')).submit(function(){
				adminFormSavedIcs( jQuery(this).attr('id') );
			});
		}
	}, 1000);

	if(jQuery('.icsInputsWithDescrForm').size()) {
		jQuery('.icsInputsWithDescrForm').find('input[type=checkbox][data-optkey]').change(function(){
			var optKey = jQuery(this).data('optkey')
			,	descShell = jQuery('#icsFormOptDetails_'+ optKey);
			if(descShell.size()) {
	...
	icsInitStickyItem();
	icsInitCustomCheckRadio();
	//icsInitCustomSelect();
	
	jQuery('.icsFieldsetToggled').each(function(){
		var self = this;
		jQuery(self).find('.icsFieldsetContent').hide();
		jQuery(self).find('.icsFieldsetToggleBtn').click(function(){
			...
			jQuery(self).find('.icsFieldsetContent').slideToggle( 300, function(){
				if(show) {
	...
	if(jQuery('#icsPopupGoToTopBtn').size()) {
		jQuery('#icsPopupGoToTopBtn').click(function(){
			jQuery('html, body').animate({
				scrollTop: 0
			}, 1000);
			jQuery(this).parents('#icsPopupGoToTop:first').hide();
			return false;
		});
	}
	...
	icsInitTooltips();
	if(jQuery('.icsCopyTextCode').size()) {
		setTimeout(function(){	// Give it some time - wait until all other elements will be initialized
			...
			jQuery('.icsCopyTextCode').attr('readonly', 'readonly').click(function(){
				this.setSelectionRange(0, this.value.length);
			}).focus(function(){
				this.setSelectionRange(0, this.value.length);
			});
			jQuery('input.icsCopyTextCode').each(function(){
...
function icsInitTooltips( selector ) {
	var tooltipsterSettings = {
		contentAsHTML: true
	,	interactive: true
	,	speed: 0
...
function changeAdminFormIcs(formId) {
	if(jQuery.inArray(formId, icsAdminFormChanged) == -1)
		icsAdminFormChanged.push(formId);
}
function adminFormSavedIcs(formId) {
	if(icsAdminFormChanged.length) {
		for(var i in icsAdminFormChanged) {
			if(icsAdminFormChanged[i] == formId) {
				icsAdminFormChanged.pop(i);
			}
		}
	}
}
function checkAdminFormSaved() {
	if(icsAdminFormChanged.length) {
		if(!confirm(toeLangIcs('Some changes were not-saved. Are you sure you want to leave?'))) {
			return false;
		}
		icsAdminFormChanged = [];	// Clear unsaved forms array - if user wanted to do this
	}
	return true;
}
...
function icsInitStickyItem() {
	jQuery(window).scroll(function(){
		...
		,	footer = jQuery('.icsAdminFooterShell')
		...
		if(wasSticking) {
			if(jQuery('#icsPopupGoToTop').size())
				jQuery('#icsPopupGoToTop').show();
		} else if(wasUnSticking) {
			if(jQuery('#icsPopupGoToTop').size())
				jQuery('#icsPopupGoToTop').hide();
		}
	});
}
function icsInitCustomCheckRadio(selector) {
	...
	,	radioClass: 'iradio_minimal'
		...
		// for checkboxHiddenVal type, see class htmlIcs
...
function icsCheckDestroy(checkbox) {
	if(!jQuery.fn.iCheck) return;
	jQuery(checkbox).iCheck('destroy');
}
function icsCheckDestroyArea(selector) {
	if(!jQuery.fn.iCheck) return;
	jQuery(selector).find('input[type=checkbox]').iCheck('destroy');
}
function icsCheckUpdate(checkbox) {
	if(!jQuery.fn.iCheck) return;
	jQuery(checkbox).iCheck('update');
}
function icsCheckUpdateArea(selector) {
	if(!jQuery.fn.iCheck) return;
	jQuery(selector).find('input[type=checkbox]').iCheck('update');
}
function icsGetTxtEditorVal(id) {
...
function icsSetTxtEditorVal(id, content) {
	if(typeof(tinyMCE) !== 'undefined' 
		&& tinyMCE 
		&& tinyMCE.get( id ) 
		&& !jQuery('#'+ id).is(':visible')
 ...
 */
function icsGridSetListSearch(param, gridSelectorId) {
 ...
 * @param {string} gridSelectorId ID of grid table html element
 */
function icsGridDoListSearch(param, gridSelectorId) {
	icsGridSetListSearch(param, gridSelectorId);
 ...
 */
function icsGetGridDataById(id, gridSelectorId) {
 ...
 */
function icsGetGridColDataById(id, column, gridSelectorId) {
	var rowId = getGridRowId(id, gridSelectorId);
	if(rowId) {
...
function icsInitPlugNotices() {
	...
	if($notices && $notices.size()) {
		$notices.each(function(){
			...
			var $enbStatsBtn = jQuery(this).find('.icsEnbStatsAdBtn');
 ...
 */
function icsGetMainPromoPopup() {
	if(jQuery('#icsOptInProWnd').hasClass('ui-dialog-content')) {
		return jQuery('#icsOptInProWnd');
	}
	return jQuery('#icsOptInProWnd').dialog({
		modal:    true
	,	autoOpen: false
	,	width: 540
	,	height: 200
	,	open: function() {
			jQuery('#icsOptWndTemplateTxt').hide();
			jQuery('#icsOptWndOptionTxt').show();
		}
	});
}
function icsInitMainPromoPopup() {
	if(!ICS_DATA.isPro) {
		var $proOptWnd = icsGetMainPromoPopup();
		jQuery('.icsProOpt').change(function(e){
			e.stopPropagation();
			var needShow = true
			...
			var parent = null;
			if(jQuery(this).parents('#icsPopupMainOpts').size()) {
				parent = jQuery(this).parents('label:first');
			} else if(jQuery(this).parents('.icsPopupOptRow:first').size()) {
				parent = jQuery(this).parents('.icsPopupOptRow:first');
			} else {
				parent = jQuery(this).parents('tr:first');
			}
			if(!parent.size()) return;
			var promoLink = parent.find('.icsProOptMiniLabel a').attr('href');
			if(promoLink && promoLink != '') {
				jQuery('#icsOptInProWnd a').attr('href', promoLink);
			}
