 ...
 */
if (!function_exists('dateToTimestampIcs')) {
	function dateToTimestampIcs( $date ) {
		if (empty($a)) {
			return false;
		}
		$a = explode(ICS_DATE_DL, $date);
...
if (!function_exists('importClassIcs')) {
	function importClassIcs( $class, $path = '' ) {
		if (!class_exists($class)) {
			if (!$path) {
				$classFile = $class;
				if (strpos(strtolower($classFile), ICS_CODE) !== false) {
					$classFile = preg_replace('/' . ICS_CODE . '/i', '', $classFile);
				}
				$path = ICS_CLASSES_DIR . $classFile . '.php';
			}
			...
			/*if(class_exists('ReflectionClass')) {   //ReflectionClass supported begining from php5
				...
				if(strpos($classFile, ICS_DIR) === false) {   //Class is not in our plugin directory
					...
					if(!empty($plugins)) {
						for($i = 0; $i < count($plugins); $i++) {
							if(strpos($plugins[$i], ICS_PLUG_NAME) !== false) {   //Let's remove our plugin from list of active plugins
								unset($plugins[$i]);
							}
						}
 ...
 */
if (!function_exists('toeGetClassNameIcs')) {
	function toeGetClassNameIcs( $class ) {
		$className = '';
		if (class_exists($class. strFirstUp(ICS_CODE))) {
			$className = $class. strFirstUp(ICS_CODE);
		} else if (class_exists(ICS_CLASS_PREFIX . $class)) {
			$className = ICS_CLASS_PREFIX . $class;
		} else {
			$className = $class;
		}
		return $className;
 ...
 */
if (!function_exists('toeCreateObjIcs')) {
	function toeCreateObjIcs( $class, $params ) {
		$className = toeGetClassNameIcs($class);
		$obj = NULL;
 ...
 */
if (!function_exists('redirectIcs')) {
	function redirectIcs( $url ) {
		if (headers_sent()) {
 ...
 * @return array
 */
if (!function_exists('prepareParamsIcs')) {
	function prepareParamsIcs( &$d=array(), &$options = array() ) {
		if (!empty($d['params'])) {
		...
		if (isset($d['code'])) {
			if ($d['code'] == '') {
				$d['code'] = prepareFieldCodeIcs($d['label']) . '_' . rand(0, 9999999);
			}
		}
		return $d;
	}
}
if (!function_exists('prepareFieldCodeIcs')) {
	function prepareFieldCodeIcs( $string ) {   
